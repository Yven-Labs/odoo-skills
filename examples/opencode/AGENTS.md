# Odoo Skills — OpenCode

Add this to your `~/.config/opencode/AGENTS.md` global instructions file.
Also copy `opencode.json` to `~/.config/opencode/opencode.json` to register the `odoo-skills` agent.
...
Shared convention files under `~/.config/opencode/skills/_shared/` provide full reference documentation (sub-agents have inline instructions — convention files are supplementary):
...
When you detect an Odoo project (presence of `__manifest__.py` or `AGENTS.md` in the repo):

1. **Load skills** from `~/.config/opencode/skills/` — the `odoo-*` skills provide domain-specific rules.
2. **Follow `AGENTS.md`** as the master router — it maps every action to the correct skill.
3. **Detect version** from `__manifest__.py` → first segment of the `version` field (e.g., `"18.0.1.0.0"` → v18). Default: **Odoo 18**.

### Skills Path

Skills are installed at `~/.config/opencode/skills/`. Each skill is a directory with a `SKILL.md` file. Load the relevant skill before writing any Odoo code.
<!-- END:odoo-skills -->
